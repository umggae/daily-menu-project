...
같은 날짜에도 카테고리마다, 그리고 점심/저녁마다 서로 다른 맛집이 나오도록 카테고리 순서와 식사 시간대를 오프셋으로 사용합니다. (`(날짜 순번 + 카테고리 순번 × 배수 + 점심/저녁 오프셋) % 목록 길이`) 매일 한국시간 10:30(점심)·16:30(저녁) 두 번 자동 갱신됩니다.

탭을 누르면 오늘의 확정 맛집으로 휠이 돌아가며, **다시 돌리기** 버튼을 누르면 매번 진짜 랜덤으로 다른 맛집이 뽑힙니다. **정문·후문·회기** 지역 필터로 휠 후보를 좁힐 수 있고(주소 기준 임의 분류), 결과 아래 **지도에서 보기**로 카카오맵 검색을 바로 열 수 있습니다. 필터링 결과가 0곳이면 빈 상태 안내로 전환됩니다.
